...
ggplot(ma_dat) +
  ...
  ## plot the analogues
  geom_point(
    data = ma_dat[analogue_data[["nn_indices"]]$topk_indices, ],
    aes(x = week_start, y = smooth_unweighted_ili),
    color = "red",
    size = 3,
    shape = 3
  ...
  geom_point(
    data = ma_dat[analogue_data[["nn_indices"]]$topk_indices + this_h, ],
    aes(x = week_start, y = smooth_unweighted_ili),
    color = "red",
    shape = 20
  ) +
  ## plot the prediction
  geom_point(
    x = ma_dat$week_start[idx + this_h],
...
ggplot(ma_dat) +
  geom_path(
    aes(y = smooth_unweighted_ili, x = smooth_unweighted_ili_lag1),
    alpha = .5
  ) +
  geom_point(
    aes(y = smooth_unweighted_ili, x = smooth_unweighted_ili_lag1),
  ...
  ## plot the analogues
  geom_point(
    data = ma_dat[analogue_data[["analogue_indices"]]$topk_indices, ],
    aes(y = smooth_unweighted_ili, x = smooth_unweighted_ili_lag1),
    color = "red",
    size = 3,
    shape = 3
  ...
  ## plot the analogues' futures
  geom_point(
    data = ma_dat[analogue_data[["analogue_indices"]]$topk_indices + this_h, ],
    aes(y = smooth_unweighted_ili, x = smooth_unweighted_ili_lag1),
    color = "red",
    shape = 20
  ) +
